 ...
 * `library-routes.ts:146`, `catalog-routes.ts:322`, `latex-routes.ts:784`.
...
export interface OrphanThumbnailSweepOverrides {
  ...
  listRows?: () => Array<{ id: string; converted_from: string | null }>;
}
 ...
 * (`paths.ts:90`) — so a keep-set that misses either half of a pair deletes a
 * live cover. Two things make `listBooks("recent")` sufficient here even
 * though it hides conversions (`NOT_CONVERTED`, `db.ts:893`):
 *
 ...
 *     row with `converted_from !== null` (`convert-jobs.ts:199`), so no chain
