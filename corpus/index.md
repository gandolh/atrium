...
- [api-layering.md](wiki/api-layering.md) — How apps/api is organised since D47 — the six domain modules, what a controller/service/model may and may not do, the tagged-union outcome convention, profile scoping in SQL, and how Knex migrations are registered.
- [architecture.md](wiki/architecture.md) — How the app is put together — the npm-workspaces monorepo (web/api/shared), the API's module/controller/service/model layering, Knex migrations, the auth guard, and the upload→store→read data flow.
...
- [decisions.md](wiki/decisions.md) — Locked tech/design decisions (D1–D47) — the settled calls future briefs and reviews must not relitigate without an explicit revisit + log note.
...
- [status-history-v1.md](wiki/status-history-v1.md) — Archive of Atrium's earliest shipped phases — the v1 build through brief 35 (profiles), July–August 2026. Split off status-history.md, which holds the more recent archive.
- [status-history.md](wiki/status-history.md) — Archive of Atrium's shipped phases through brief 44 — what each run delivered, what it fixed, and how it was verified. Split out of status.md, which keeps the current snapshot.
